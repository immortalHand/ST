				...
				<table class="tb">
					...
					<c:forEach items="${szList }" var="sz" varStatus="status">
						<tr <c:if test="${status.index%2 == 0 }">style='background-color:#f1f2f3'</c:if>>
							...
							<td>${sz.phone }</td>
							<td>
								<a href="${pageContext.request.contextPath }/user/houshezhangupdate.do?id=${sz.id}"></a>
								<a onclick="deluser('${sz.id}')"></a>
								<a href="${pageContext.request.contextPath }/user/renshez.do?id=${sz.id }">任命</a>
							</td>
						</tr>
					</c:forEach>
